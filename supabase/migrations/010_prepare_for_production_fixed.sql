-- Migration: Prepare for Production Features (FIXED VERSION)
-- Date: 2025-01-29
...
-- ============================================
-- 6. SYSTEM SETTINGS TABLE (CREATE FIRST)
-- ============================================
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can view invitations they sent or received' AND tablename = 'session_invitations') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Session owners can send invitations' AND tablename = 'session_invitations') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Participants can view collaboration events' AND tablename = 'collaboration_events') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can view tool calls in their sessions' AND tablename = 'ai_tool_calls') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Anyone can read search cache' AND tablename = 'web_search_cache') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can view their own OAuth connections' AND tablename = 'user_oauth_connections') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can manage their own OAuth connections' AND tablename = 'user_oauth_connections') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Settings are publicly readable' AND tablename = 'settings') THEN
...
DROP TRIGGER IF EXISTS update_user_last_seen ON user_activities;
...
-- ============================================
-- 10. INSERT DEFAULT SETTINGS DATA
-- ============================================
