...
interface Props {
  ...
  onUpdateNode?: (nodeId: string, updates: { prompt?: string; metadata?: any }) => void
}

export function NodeDetailSidebar({ node, allNodes, isOpen, onClose, session, onModelChange, onWidthChange, onRetryNode, onDeleteNode, onUpdateNode }: Props) {
                ...
                <div className="space-y-3">
                  ...
                  {noteContent !== currentDisplayNode.prompt && (
                    <div className="text-xs text-gray-500 italic">
                      Changes will be saved automatically when you switch nodes or close the sidebar
                    </div>
                  )}
                </div>
              ) : (
                /* Regular user prompt - display only */
